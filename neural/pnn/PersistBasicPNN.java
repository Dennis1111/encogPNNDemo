...
/**
 * Persist a PNN. Needs to be fixed assumes separateClasses is not used
 */
public class PersistBasicPNN implements EncogPersistor {
	...
	public static final String USE_PRIORS = "usePriors";

	
	public boolean saveSamples = true;
	...
	public Object read(final InputStream is) {
		...
		while ((section = in.readNextSection()) != null) {
			...
			if (section.getSectionName().equals("PNN") && section.getSubSectionName().equals("NETWORK")) {
				...
				usePriors = section.parseBoolean(params, USE_PRIORS);
			}
		...
		System.out.println("ReadSigma" + Arrays.toString(sigma));
		boolean sepClasses=inputCount*outputCount==sigma.length;
		final BasicPNN result = new BasicPNN(kernel, outmodel, inputCount, outputCount, sepClasses,usePriors);
	...
	public void save(final OutputStream os, final Object obj) {
		...
		out.writeProperty(USE_PRIORS, pnn.usePriors());
		out.addSubSection("SAMPLES");

		if (pnn.getSamples() != null && saveSamples) {

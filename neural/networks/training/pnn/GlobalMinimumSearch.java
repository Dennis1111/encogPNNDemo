...
public class GlobalMinimumSearch {
	...
	public double brentmin(final int maxIterations, final double maxError, final double eps, final double tol,
			final CalcError errorCrit, final double y) {
		double prevdist = 0.0;
		double step = 0.0;
		...
		for (int iter = 0; iter < maxIterations; iter++) {
			 ...
			 * function.
			 */
			final double frecent = errorCrit.calcError(xrecent);
	...
	public void findBestRange(final double low, final double high, int numberOfPoints, final boolean useLog,
			final double minError, final CalcError errorCrit,boolean firstPointKnown)
	...
	{		
		...
		// Skip this at the moment, for some examples this can get crazy
		//gettingWorse = true;
		if (!gettingWorse) {
			...
			for (;;) {
				...
				this.y3 = errorCrit.calcError(this.x3);

				// Use x or sigmaCandidate ? should we modify x ?
		...
		// search space. Lets try those.
			...
			for (;;) {
				...
				this.y1 = errorCrit.calcError(this.x1);
				if (this.y1 < 0.0) {
					return;
				}
